...
class CrossReferenceModelTest(TestCase):
    def setUp(self):
        ...
        self.v1 = Verse.objects.create(book=self.book, version=self.version, chapter=1, number=1, text="In the beginning...")
        self.v2 = Verse.objects.create(book=self.book, version=self.version, chapter=1, number=2, text="And the earth...")

    def test_crossref_creation(self):
        cr = CrossReference.objects.create(from_verse=self.v1, to_verse=self.v2, relationship_type="parallel", source="manual")
    ...
    def test_crossref_no_self_reference(self):
        with self.assertRaises(IntegrityError):
            CrossReference.objects.create(from_verse=self.v1, to_verse=self.v1, relationship_type="other", source="manual")
